 ...
 * @version 1
 */
public class HAStarPuzzleState implements Comparable<HAStarPuzzleState>{
	 ...
	 * @param board
	 */
	public HAStarPuzzleState(String board, int newMode) {
		boardState = board;
		mode = newMode;
		depth = 0;
		h1 = 0;
	 ...
	 */
	public Queue<HAStarPuzzleState> getSucc() {
		
		Queue<HAStarPuzzleState> succ = new LinkedList<HAStarPuzzleState>();
	 ...
	 */
	private void addSucc(int blank, int newIndex, Queue<HAStarPuzzleState> succ) {
		...
		HAStarPuzzleState newSucc = new HAStarPuzzleState(s1 + s.charAt(newIndex) + s2 + s.charAt(blank) + s3, mode);
		succ.add(newSucc);
	}
	 ...
	 */
	public int compareTo(HAStarPuzzleState s2) {
		if (mode == 1) { // Heuristic 1
			if (this.getH1() + depth < s2.getH1() + depth) {
				return -1;
			} else if (this.getH1() + depth > s2.getH1() + depth) {
				return 1;
			}
		} else {         // Heuristic 2
			if (this.getH2() + depth < s2.getH2()+ depth) {
				return -1;
			} else if (this.getH2() + depth > s2.getH2() + depth) {
				return 1;
			}
		}
